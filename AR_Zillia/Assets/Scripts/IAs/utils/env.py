...
FAQ_PATH = Path(PASTA_DATA) / "infos" / "FAQ.txt"
...
def carregar_faq_rma():
    texto = FAQ_PATH.read_text(encoding="utf-8")
    ...
    for linha in texto.splitlines():
        ...
        if linha.startswith("pergunta:"):
            pergunta = linha[9:].strip()

        elif linha.startswith("resposta:"):
            resposta = linha[9:].strip()
            faqs.append({
                "pergunta": pergunta,
                "resposta": resposta
            })
